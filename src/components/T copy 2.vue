...
const regionSortBy = ref({ weekId: null, columnKey: 'rank', direction: 'asc' })
const storeSortBy = ref({ weekId: null, columnKey: 'rank', direction: 'asc' })
...
const calculateWeeklyRanks = () => {
  if (!salesData.value || !regions.value) return

  salesData.value.weeks.forEach(week => {
    console.log(`🏆 Рассчитываем ранги для ${week.name}...`)
    ...
    // Собираем все объекты (регионы + магазины) для данной недели
    const allEntities = []
    ...
    // Сортируем по totalScore (по убыванию - лучший результат = ранг 1)
    ...
    allEntities.forEach((item, index) => {
      ...
      if (item.type === 'region') {
        // console.log(`📊 Регион ${item.entity.name}: ранг ${rank}, балл ${item.totalScore}`)
      }
    })
    ...
    storeEntities.sort((a, b) => b.totalScore - a.totalScore)

    storeEntities.forEach((item, index) => {
      const storeRank = index + 1
      item.weekData.rank = storeRank
    })
...
function getStyle(key) {
  ...
  return {
    ...
    borderRight: isVisible ? '1px solid #91b6db!important' : 'none!important',
  }
}
...
// Функция экспорта дашборда в Excel
const exportToExcel = () => {
  if (!salesData.value || !regions.value || !weeks.value) {
  ...
  try {
    ...
    // Лист 1: Сводная таблица по регионам
    createRegionsSummarySheet(workbook)
    
    // Лист 2: Детальные данные по магазинам
    createStoresDetailSheet(workbook)
    
    // Лист 3: Планы и настройки
    createPlansSheet(workbook)
    
    // Лист 4: KPI и аналитика
    createKPISheet(workbook)
    
    // Генерируем имя файла
    const periodName = selectedPeriod.value.replace(/\s+/g, '_')
    const dateStr = new Date().toISOString().slice(0, 10)
    const fileName = `Dashboard_SMK_${periodName}_${dateStr}.xlsx`
    
    // Сохраняем файл
    XLSX.writeFile(workbook, fileName)
    
    showNotification(`Excel файл сохранен: ${fileName}`, 'success')
...
// Создание листа сводной таблицы по регионам
const createRegionsSummarySheet = (workbook) => {
  const data = []
  
  // Заголовки
  const headers = ['Регион']
  weeks.value.forEach(week => {
    availableIndicators.value.forEach(indicator => {
      headers.push(`${week.name} - ${indicator.label}`)
    })
  })
  headers.push('Общий балл', 'Ранг')
  data.push(headers)
  
  // Данные по регионам
  sortedRegions.value.forEach(region => {
    const row = [region.name]
    
    weeks.value.forEach(week => {
      availableIndicators.value.forEach(indicator => {
        const value = getRegionData(region, week.id, indicator.key)
        row.push(value === '-' ? 0 : value)
      })
    })
    
    row.push(region.overallTotalScore || 0, region.regionRank || 0)
    data.push(row)
  })
  ...
  const colWidths = [{ wch: 20 }] // Колонка "Регион"
  weeks.value.forEach(() => {
    availableIndicators.value.forEach(() => {
      colWidths.push({ wch: 12 })
    })
  })
  colWidths.push({ wch: 12 }, { wch: 8 }) // Общий балл, Ранг
  
  worksheet['!cols'] = colWidths
  
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Регионы')
}

// Создание детального листа по магазинам
const createStoresDetailSheet = (workbook) => {
  const data = []
  
  // Заголовки
  const headers = ['Регион', 'Магазин', 'ID магазина']
  weeks.value.forEach(week => {
    availableIndicators.value.forEach(indicator => {
      headers.push(`${week.name} - ${indicator.label}`)
    })
  ...
  allStores.value.forEach(store => {
    const row = [
      store.regionName || 'Неизвестно',
      store.name || 'Неизвестно',
      store.id.replace('store_', '')
    ]
    
    weeks.value.forEach(week => {
      availableIndicators.value.forEach(indicator => {
        const value = getStoreData(store, week.id, indicator.key)
        row.push(value === '-' ? 0 : value)
      })
    })
    
    row.push(store.overallTotalScore || 0, store.overallRank || 0)
    data.push(row)
  })
  ...
  worksheet['!cols'] = [
    ...
    { wch: 25 }, // Магазин  
    { wch: 12 }, // ID
    ...Array(headers.length - 5).fill({ wch: 10 }), // Данные
    { wch: 12 }, // Общий балл
    { wch: 8 }   // Ранг
  ]
  
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Магазины')
}

// Создание листа с планами
const createPlansSheet = (workbook) => {
  if (!targetsData.value) return
  
  const data = []
  
  // Настройки показателей
  data.push(['НАСТРОЙКИ ПОКАЗАТЕЛЕЙ'])
  data.push(['Показатель', 'Название', 'Макс. балл', 'Тип'])
  
  Object.entries(targetsData.value.targetTree).forEach(([key, target]) => {
    data.push([key, target.name, target.maxScore, target.type])
  })
  
  data.push([]) // Пустая строка
  
  // Планы по магазинам
  data.push(['ПЛАНЫ ПО МАГАЗИНАМ'])
  const planHeaders = ['ID магазина', 'Название']
  const indicators = Object.keys(targetsData.value.targetTree).filter(key => key !== 'turnover')
  planHeaders.push(...indicators.map(key => targetsData.value.targetTree[key].name))
  data.push(planHeaders)
  
  Object.entries(targetsData.value.storeTargets).forEach(([storeId, storeData]) => {
    const row = [
      storeId,
  ...
  worksheet['!cols'] = [
    { wch: 12 }, // ID
    { wch: 25 }, // Название
    ...Array(indicators.length).fill({ wch: 12 }) // Показатели
  ]
  
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Планы')
}

// Создание листа с KPI
const createKPISheet = (workbook) => {
  if (!processedData.value) return
  
  const data = []
  
  // Общая сводка
  data.push(['ОБЩАЯ СВОДКА'])
  data.push(['Показатель', 'Значение'])
  data.push(['Всего магазинов', processedData.value.totalStores])
  data.push(['Регионов', processedData.value.totalRegions])
  data.push(['Средний балл', processedData.value.averageScore])
  data.push(['Выполнение плана (%)', processedData.value.planExecutionPercent])
  
  data.push([]) // Пустая строка
  
  // Топ регионы
  data.push(['ТОП-5 РЕГИОНОВ'])
  data.push(['Место', 'Регион', 'Балл'])
  processedData.value.topRegions.forEach((region, index) => {
    data.push([index + 1, region.name, Math.round(region.score)])
  })
  
  data.push([]) // Пустая строка
  
  // Топ магазины
  data.push([`ТОП-${KPITopStores.value} МАГАЗИНОВ`])
  data.push(['Место', 'Магазин', 'Регион', 'Балл'])
  processedData.value.topStores.forEach((store, index) => {
    data.push([index + 1, store.name, store.regionName, Math.round(store.overallTotalScore)])
  })
  
  data.push([]) // Пустая строка
  
  // Проблемные зоны
  data.push(['ПРОБЛЕМНЫЕ ЗОНЫ'])
  data.push(['Показатель', 'Значение', 'Магазинов'])
  data.push(['В зоне риска', processedData.value.problemStores.length, ''])
  data.push(['Не выполняют план', processedData.value.belowPlanStores.length, ''])
  
  processedData.value.topIssues.forEach(issue => {
    data.push([issue.name, formatNumber(issue.totalValue), issue.affectedStores])
  })
  
  const worksheet = XLSX.utils.aoa_to_sheet(data)
  worksheet['!cols'] = [
    { wch: 20 }, // Показатель/Место
    { wch: 30 }, // Значение/Название
    { wch: 15 }  // Доп. данные
  ]
