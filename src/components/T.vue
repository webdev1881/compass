      ...
      <div class="odx-color-palette__content">
        ...
        <div class="odx-format-controls">
          ...
          <label class="odx-toggle">
            ...
            <div class="odx-toggle__label">Форматування</div>
          </label>
        </div>
      </div>
    </div>
...
const loadData = async () => {
  try {
    ...
    const dateRange = getDateRangeForPeriod('Два місяці', today.value)
...
function getStyle(key) {
  ...
  if (key === 'totalScore' || key === 'rank') {
    width = `${staticWidth.value}px`
  } else {
    width = isVisible ? `${100 / total}%` : '0%'
  }

  return {
    width,
    transform: isVisible ? 'width:100%' : 'width:0%',
...
const getStoreData = (store, weekId, indicator) => {
  ...
  switch (indicator) {
    ...
    case 'percent': return weekData.percent ? `${weekData.percent}%` : '-'
    case 'turnover_score': return weekData.turnover_score || '-'
    default:
      if (indicator.endsWith('_percent')) {
        return weekData[indicator] ? `${weekData[indicator]}%` : '-'
      } else if (indicator.endsWith('_score')) {
        return weekData[indicator] || '-'
      } else {
        return formatNumber(weekData[indicator] || '-')
...
const getCellClass = (indicator, weekData, isRegion = false, weekId = null, region = null, cellValue = null) => {
  const classes = []

  if (cellValue === null) {
    if (isRegion && region) {
      cellValue = getRegionData(region, weekId, indicator)
    } else {
      cellValue = getStoreData({ weeklyData: [weekData] }, weekId, indicator)
    }
  }
  ...
  if (cellValue === '-' || cellValue === 0 || cellValue === '0' || cellValue === '0%' ||
    ...
    if (indicator === 'rank') {
      classes.push('odx-table__cell--rank')
    }
...
const exportToExcel = () => {
  ...
  try {
    ...
    showNotification(`Excel файл збережено: ${fileName}`, 'success')
...
const createStoresDetailSheet = (workbook) => {
  ...
  worksheet['!cols'] = [
    ...
    { wch: 25 },
    { wch: 12 },
    ...Array(headers.length - 5).fill({ wch: 10 }),
    { wch: 12 },
    { wch: 8 }
  ]
...
const createPlansSheet = (workbook) => {
  ...
  worksheet['!cols'] = [
    { wch: 12 },
    { wch: 25 },
    ...Array(indicators.length).fill({ wch: 12 })
  ]
